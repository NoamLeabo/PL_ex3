...
let rec fv term =
  match term with
  | Variable x -> StringSet.singleton x  (* FV(x) = {x} *)
  | Abstraction (x, t) -> StringSet.remove x (fv t)  (* FV(λx. t) = FV(t) - {x} *)
  | Application (t1, t2) -> StringSet.union (fv t1) (fv t2)  (* FV(t1 t2) = FV(t1) ∪ FV(t2) *)
...
let rec reduce_cbv term =
	...
	| Parser.Abstraction _ -> None  (* Abstractions are values and cannot be reduced *)
	| Parser.Application (Parser.Abstraction (x, body), t2) ->
			(* (λx. body) t2: Reduce if t2 is a value *)
...
let rec reduce_cbn term =
	match term with
	| Parser.Variable _ -> None  (* Variables are irreducible *)
	| Parser.Abstraction _ -> None  (* Abstractions are values *)
	| Parser.Application (Parser.Abstraction (x, body), t2) ->
			(* Perform substitution directly, without reducing t2 *)
			Some (substitute x t2 body)
	| Parser.Application (t1, t2) ->
			(* First, try reducing t1 *)
